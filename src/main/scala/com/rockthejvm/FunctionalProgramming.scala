...
object FunctionalProgramming extends App:
   ...
   *
   * val doubler: Function1[Int, Int] = new Function1[Int, Int] { override def
   * apply(x: Int) = 2 * x } */
  ...
  val aFlatMappedList = List(1, 2, 3).flatMap(x => List(x, 2 * x)) // alternative syntax, same as .flatMap(x => List(x, 2 * x))
